...
class ExampleUnitTest {
    ...
    @Test
    fun dropLastUntil() {
        val actual1 = listOf(1, 2, 3).dropLastUntil { it == 2 }
        Assert.assertEquals(listOf(1), actual1)

        val actual2 = "House Nymeros Martell of Sunspear".split(" ")
            .dropLastUntil { it == "of" }
        Assert.assertEquals(listOf("House", "Nymeros", "Martell"), actual2)
    }

    @Test
    fun importUsers() {
        val users = UserHolder.importUsers(listOf(" John Doe ;JohnDoe@unknow.com;[B@7591083d:c6adb4becdc64e92857e1e2a0fd6af84;;"))
